...
describe('audioEngine', () => {
  ...
  afterAll(async () => {
    ...
    const reportContent = `# Báo Cáo Kiểm Thử Audio Engine & Quản Lý Phát Nhạc Trực Tiếp
...
`;
    ...
    const reportPath = path.join(reportDir, 'audio-engine-report.md');
    fs.writeFileSync(reportPath, reportContent, 'utf8');
    console.log(`Đã xuất báo cáo AudioEngine ra tệp: ${reportPath}`);
  });
});
